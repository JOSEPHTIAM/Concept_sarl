...
const Impressions3 = () => {
  ...
  const impressionData = flatData.filter(
    item => item.type && item.type.toUpperCase() === "IMPRESSION"
  );
    
  const itemsToShow = isMobile ? 1 : 3;
  const visibleItems = impressionData.slice(itemIndex, itemIndex + itemsToShow);

  const handleNextItem = () => {
    if (itemIndex + itemsToShow < impressionData.length) setItemIndex(prev => prev + 1);
    else setItemIndex(0); 
  };

  const handlePrevItem = () => {
    if (itemIndex > 0) setItemIndex(prev => prev - 1);
    else setItemIndex(Math.max(0, impressionData.length - itemsToShow));
  };
  ...
  return (
    <section className="position-relative overflow-hidden" style={{ backgroundColor: '#fcfcfc' }}>
      ...
      <motion.div initial={{ opacity: 0, y: -100 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}
        style={{ position: 'absolute', top: 0, left: 0, right: 0, backgroundColor: '#ffffff', height: '15%', borderRadius: '0 0 100% 100% / 0 0 25% 25%', zIndex: 0 }} />
      
      <motion.div initial={{ opacity: 0, y: 100 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.4 }}
        style={{ position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#dc3545', height: '10%', borderRadius: '100% 100% 0 0 / 35% 35% 0 0', zIndex: 0 }} />
      ...
      <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <motion.div variants={fadeInUp} className="row mb-5">
            <div className="col-12">
              <h1 className="text-white fw-bold text-uppercase" style={{ fontSize: 'clamp(32px, 5vw, 52px)', lineHeight: '1.1' }}>
                <font style={{color: '#dc3545'}}>Services d&apos;impression</font>
              </h1>
            </div>
          </motion.div>
            ...
            <motion.div variants={fadeInUp} className="col-lg-6 d-flex">
              <div style={glassStyle}>
                <div style={textBodyStyle}>
                  <p><strong style={{ color: "#0b023b" }}>Maîtrisez l&apos;impact de votre message</strong> en structurant vos projets créatifs avec une rigueur professionnelle. Nous transformons vos idées en concepts visuels percutants, garantissant que chaque détail renforce votre expertise.</p>
                </div>
              </div>
            </motion.div>
            <motion.div variants={fadeInUp} className="col-lg-6 d-flex">
              <div style={glassStyle}>
                <div style={textBodyStyle}>
                  <p><strong style={{ color: "#0b023b" }}>Sublimez votre identité visuelle</strong> en créant une image de marque forte et mémorable. Notre approche fusionne esthétique moderne et efficacité technologique pour un prestige visuel qui s&apos;impose.</p>
                </div>
              </div>
            </motion.div>
          </div>
        ...
        {/* DOUBLE PAGINATION : CHIFFRES ET DOTS */}
        <div className="d-flex flex-column align-items-center" style={{ marginTop: '40px' }}>
          <div style={{ fontWeight: 'bold', fontSize: '18px', color: '#0b023b', marginBottom: '10px' }}>
             {impressionData.length > 0 ? `${itemIndex + 1} - ${Math.min(itemIndex + itemsToShow, impressionData.length)} / ${impressionData.length}` : "0 / 0"}
          </div>
          <div className="d-flex gap-2">
            {impressionData.map((_, idx) => (
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: false }} variants={staggerContainer} className="row justify-content-center" style={{ marginTop: '120px', marginBottom: '80px' }}>
          <motion.div variants={fadeInUp} className="col-lg-12 mb-5">
            <h1 className="text-danger fw-bold text-uppercase" style={{ fontSize: '48px', lineHeight: '1.1' }}>
              Visions et engagements : Impression
            </h1>
          </motion.div>
          ...
          <div className="row g-5 align-items-stretch">
            <motion.div variants={fadeInUp} className="col-lg-6 d-flex">
              <div style={glassStyle}>
                <div style={textBodyStyle}>
                  <p><strong style={{ color: "#0b023b" }}>Fidélité Chromatique :</strong><br /> Nous garantissons une reproduction des couleurs d&apos;une précision absolue grâce à nos protocoles de calibration régulière, assurant l&apos;intégrité de votre marque.</p>
                  <p><strong style={{ color: "#0b023b" }}>Éco-Responsabilité :</strong><br /> Nous privilégions l&apos;utilisation d&apos;encres écologiques et de papiers certifiés durables (FSC/PEFC), valorisant l&apos;image éthique de votre entreprise.</p>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div variants={fadeInUp} className="col-lg-6 d-flex">
              <div style={glassStyle}>
                <div style={textBodyStyle}>
                  <p><strong style={{ color: "#0b023b" }}>Réactivité Opérationnelle :</strong><br /> Nous optimisons nos flux pour vous offrir des délais records. Notre agilité est votre meilleur atout pour répondre aux urgences de votre calendrier.</p>
                  <p><strong style={{ color: "#0b023b" }}>Finitions de Prestige :</strong><br /> Pelliculage, vernis sélectif ou gaufrage : ces détails tactiles transforment un simple imprimé en un objet d&apos;exception haut de gamme.</p>
                </div>
              </div>
            </motion.div>
          </div>
